    ...
    context 'when results exist' do
      ...
      describe 'body' do
        ...
        describe 'metadata' do
          ...
          it 'returns an empty bucket for JSON aggregations not present in the documents' do
            audience_buckets = metadata['aggregations']['audience']['buckets']
            expect(audience_buckets.flatten).to eq([])
          end

          it 'returns non-empty bucket for JSON aggregations that are present in the documents' do
            tags_buckets = metadata['aggregations']['tags']['buckets']
            expect(tags_buckets.flatten).not_to eq([])
          end
        end
      end
    end
